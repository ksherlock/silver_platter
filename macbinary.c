...
Word MacBinary(struct qEntry *q) {
  ...
  header.creationDate =
      swap32(ConvSeconds(TimeRec2Secs, 0, (Pointer)&OpenDCB.createDateTime));

  header.modificationDate =
      swap32(ConvSeconds(TimeRec2Secs, 0, (Pointer)&OpenDCB.modDateTime));
  ...
  size = ((OpenDCB.eof + 127) & 0xffffff80) +
         ((OpenDCB.resourceEOF + 127) & 0xffffff80) + 128;

  SendHeader(q, 200, size, &OpenDCB.modDateTime, "application/x-macbinary",
             NULL, 0);
