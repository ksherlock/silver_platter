...
Word GetNextVolume(Word cookie, VolumeRecGS *VolumeDCB) {
  ...
  for (;; cookie++, i <<= 1) {
    ...
    if ((fDirRemovable == false) && (rcache[j] & i))
      continue;
    ...
    if (DInfoDCB.characteristics & dcBlockDevice == 0)
      continue;
    ...
    if ((VolumeDCB->fileSysID == appleShareFSID) && (fDirAppleShare == false))
      continue;
