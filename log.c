...
Word CreateLog(void) {
  ...
  if (fLog && fLogDir) {
    ...
    i = sprintf(gstr->text, "%" PRIB "/www%04u%02u%02u.txt", B(fLogDir),
                tr.year + 1900, tr.month + 1, tr.day + 1);
...
void SaveLog(Word MyID, Handle teH) {
  ...
  SFPutFile2(20, 20, refIsPointer, (Ref) "\pSave Log File", refIsPointer,
             (Ref) "\x07\x00www.log", &myReply);
  ...
  if (myReply.good) {
    ...
    if (_toolErr)
      ...
      do {
        total = TEGetText(teDataIsTextBlock + refIsHandle * 8, (Ref)h, 0, 0,
                          (Ref)0, teH);
